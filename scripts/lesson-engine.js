(function () {
  ...
  function downloadPdf() {
    ...
    mcQuestions.forEach((item, index) => {
      ...
      wrapPdfText(`Response: ${answer}`, 80).forEach(text => add(text));
    });
    add('');
    ...
    const fileName = `${(state.studentName || 'student').replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '')}-${(config.title || 'pen').replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '')}.pdf`;
  ...
  function bindGlobalActions() {
    ...
    if (printButton) {
      ...
      saveState('PDF downloaded – keep a copy with your evidence.');
      downloadPdf();
      });
    }
